defmodule Words do
  ...
  def count(sentence) do
    ...
    str = String.replace( str, ~r/[^\w -]/, "") # remove all non letter characters, but preserve spaces and dashes.
    str = String.replace( str, "  ", " ") # replace all double spaces with single spaces.
    str = Enum.map( String.split(  str , " " ), fn x -> if String.length(x) >0, do: x end)  # split the wordds by using the space as a delimiter.
    str = List.delete( str, nil ) # remove any value that has nil for a word.
